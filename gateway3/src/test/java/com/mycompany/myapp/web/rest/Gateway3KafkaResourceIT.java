...
class Gateway3KafkaResourceIT {
    ...
    void setup() {
        ...
        Gateway3KafkaResource kafkaResource = new Gateway3KafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/gateway-3-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/gateway-3-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
