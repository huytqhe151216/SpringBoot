...
class MicroserviceKafkaResourceIT {
    ...
    void setup() {
        ...
        MicroserviceKafkaResource kafkaResource = new MicroserviceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/microservice-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/microservice-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
